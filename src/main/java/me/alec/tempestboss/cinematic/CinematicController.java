...
public class CinematicController {
    ...
    public void intro(Location center, LivingEntity boss, List<Player> players, Runnable done) {
        if (!plugin.getConfig().getBoolean("cinematic.enabled", true)) { done.run(); return; }
        ...
        for (Player p : players) {
            p.showTitle(Title.title(
                    ...
                    Component.text("A storm awakens..."),
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (w != null) {
                    ...
                    if (t % 10 == 0) Particles.ring(w, center.clone().add(0, 0.2, 0), 1.8, 28);
                }
    ...
    public void phaseTransition(Location center, LivingEntity boss, List<Player> players, int newPhase, Runnable done) {
        ...
        for (Player p : players) {
            p.playSound(boom);
            p.showTitle(Title.title(
                    Component.text("PHASE " + newPhase).color(net.kyori.adventure.text.format.NamedTextColor.AQUA),
                    Component.text("The boss shifts..."),
                    Title.Times.times(Duration.ofMillis(150), Duration.ofSeconds(1), Duration.ofMillis(250))
            ));
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (w != null) {
                    ...
                    Particles.burst(w, center.clone().add(0, 1.2, 0), 28);
                }
