...
public class LootManager {
    ...
    public List<ItemStack> getLoot(BossType type) {
        String mode = plugin.getConfig().getString("loot.mode", "CUSTOM").toUpperCase(Locale.ROOT);
        if (mode.equals("NONE")) return List.of();
        if (mode.equals("DEFAULT")) return List.of();
        return getLootRaw(type);
    }
